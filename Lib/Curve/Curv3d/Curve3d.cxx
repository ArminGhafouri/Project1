...
Curve3d::Curve3d(int deg, std::vector<Pt3d> pts)
{
	theDegree_ = deg;
	thePts_ = pts;
...
void Curve3d::Plot(std::string filename, int n)
{
...
std::vector<int> Curve3d::CalcMutiplicities() const
{
...
std::vector<double> Curve3d::CalcKnots() const
{
...
Handle(Geom_Curve) Curve3d::CreateOCCurve() const
{
	...
	{
		double x = thePts_[i].GetX();
		double y = thePts_[i].GetY();
		double z = thePts_[i].GetZ();
